...
function crearInputDecimal(id, nombrePropiedad, readonly) {
    ...
    var claseMaskDecimales = nombrePropiedad === 'CostoVenta' ? 'input-money-4': 'input-money';
...
$("#btnNuevoDetalle").click(function () {
    ...
    if (compraImportacion) {
        $table.append(`<tr id="${index}"><td>${deleteButton}</td><td>${inputNombreProducto}</td><td>${selectLocalBodega}</td><td>${spinnerCantidad}</td>
                <td>${selectTipoUnidad}</td><td>${inputValorFOB}</td><td>${inputFactorDistribucion}</td><td>${inputCostoUnitarioImportacion}</td>
                <td>${inputCostoVenta}</td><td>${inputCaducidad}</td><td>${inputSubtotal}</td></tr>`);
    }
    else {
        ...
        $table.append(`<tr id="${index}"><td>${deleteButton} ${inputValorFOB} ${inputCostoUnitarioImportacion} ${inputCostoVenta}</td>
                ...
                <td>${selectTipoUnidad}</td><td>${inputFactorDistribucion}</td><td>${inputSubtotal}</td></tr>`);
    }
    /////////////////////////////////////////////////////
...
function recalcularTotales(element) {
    ...
    var cantidad = $($tr.find('.text-cantidad:first')).toFloat();
    //var costoVenta = parseFloat($tr.find('.text-CostoVenta:first').maskMoney('unmasked')[0]);
    var costoVenta = $($tr.find('.text-FactorDistribucion:first')).toFloat();
    var subtotal = $((cantidad * costoVenta) ).toFloat();
