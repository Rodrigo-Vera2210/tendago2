var ClienteData = [];
var CierreCaja = function () {

    var seleccionarCliente = function () {
        var $urlCliente = CierreCaja.searchCustomerUrl;
    ...
    var getPaymentList = function () {
        var medios = CierreCaja.paymentMethods;
    ...
    var saveReceipt = function () {
        ...
        if (!$form.validate()) {
            swal("Debe llenar los datos solicitados!", "", "error");
            return false;
        }
            ...
            .done(function (respuesta) {
                if (respuesta) {
                    if (respuesta.success) {
                        ...
                        $(".text-cierreid").text(respuesta.result.Id)
                    ...
                    toastr.error("Hubo un error al guardar cierre de caja!", "", optionsToastr);
                }
    ...
    var previewHandler = function () {
        $form = $("#guardarCierreForm");
        var $data = $("#tab1 :input").serialize();

        $.post(CierreCaja.detailsReceiptUrl, $data)
            .done(function (respuesta) {
                if (respuesta) {
                    if (respuesta.includes("login-form")) {
                        swal("", "Debe iniciar sesión para continuar", "error");
                        window.open("/auth", "_login", "");
                    }
                    else {
                        $("#detalleCaja").empty().html(respuesta);
                    }
                }
                else {
                    swal("", "Hubo un error al cargar los detalles", "error");
                }
            });

        $.post(CierreCaja.previewReceiptUrl, $data)
            .done(function (respuesta) {
                if (respuesta) {
                    if (respuesta.includes("login-form")) {
                        swal("", "Debe iniciar sesión para continuar", "error");
                        ...
                        $("#resumenCaja").empty().html(respuesta);
                    }
                }
                else {
                    swal("", "Hubo un error al cargar los detalles", "error");
    ...
    return {
        ...
        configure: function () {
            var $wizard = $("#guardarCierreForm").formwizardcustom();

            $(".datepicker").attr("data-format", "yyyy-mm-dd");
            $(".datepicker").datepickercustom();

            $(".finish > button").on("click", saveReceipt);
