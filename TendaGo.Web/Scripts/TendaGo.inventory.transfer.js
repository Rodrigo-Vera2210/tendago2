...
var Transfer = function () {
    ...
    var processing = false,
        ...
        main = function (preload) {
            ...
            ajaxLocal = $.get(`/inventario/GetLocales/${$id_producto}`, function ($locales) {
                ...
                if ($locales.length > 0) {
                    ...
                    for (var i = 0; i < $locales.length; i++) {
                        var $local = $locales[i];
                        $ctrl.append(`<option value="${$local.IdLocal}" data-stock="${$local.StockInventario || 0}"  >${$local.Local}   |  Stock: ${$local.StockInventario || 0}  |  ${$local.NombreUnidadMedida} </option>`);
                    }
            ...
            if ($tr.length) {
                var $last = $tr.last();
                var $uid = $last.attr("data-uid");

                $id = $uid ? parseInt($uid) + 1 : 0;
            } 
