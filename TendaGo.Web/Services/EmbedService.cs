...
namespace TendaGo.Web.Services
{
    ...
    {
        private static readonly string AuthorityUrl = ConfigurationManager.AppSettings["report:authorityUrl"];
        private static readonly string ResourceUrl = ConfigurationManager.AppSettings["report:resourceUrl"];
        private static readonly string ApplicationId = ConfigurationManager.AppSettings["report:applicationId"];
        private static readonly string ApiUrl = ConfigurationManager.AppSettings["report:apiUrl"];
        private static readonly Guid WorkspaceId = new Guid(ConfigurationManager.AppSettings["report:workspaceId"]);
        private static readonly Guid ReportId = new Guid(ConfigurationManager.AppSettings["report:reportId"]);

        private static readonly string AuthenticationType = ConfigurationManager.AppSettings["report:AuthenticationType"];
        ...
        public async Task<bool> EmbedReport(string username, string roles, params string[] filters)
        {
            ...
            try
            {
                ...
                using (var client = new PowerBIClient(new Uri(ApiUrl), m_tokenCredentials))
                {
                    ...
                    var datasets = await client.Datasets.GetDatasetAsync(WorkspaceId, report.DatasetId);
                    ...
                    var tokenResponse = m_embedConfig.EmbedToken; //await client.Reports.GenerateTokenInGroupAsync(WorkspaceId, report.Id, generateTokenRequestParameters);
                    ...
                    if (filters != null)
                    {
                        ...
                        parameters = $"&$filter={sb}";
                    }

                    // Generate Embed Configuration.
                    // m_embedConfig.EmbedToken = tokenResponse;
                    m_embedConfig.EmbedUrl = $"{report.EmbedUrl}{parameters}";
        ...
        private async Task<bool> GetTokenCredentials()
        {
            ...
            m_tokenCredentials = new TokenCredentials(authenticationResult.AccessToken, authenticationResult.AccessTokenType);
            Guid.TryParse(authenticationResult.UserInfo.UniqueId, out var tokenId);
            m_embedConfig.EmbedToken = new EmbedToken
            {
                Expiration = authenticationResult.ExpiresOn.DateTime,
