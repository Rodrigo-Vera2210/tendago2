   ...
   public class EntityUtils
    {
        ...
        public static object ByteArrayToObject(Byte[] buffer)
        {
         ...
         public static object ConvertirATipoDato(Type PropertyType, object Dato)
        {
            if (PropertyType == typeof(short))
                return Convert.ToInt16(Dato);
            if (PropertyType == typeof(int))
                ...
                return Convert.ToSByte(Dato);
            else if (PropertyType == typeof(decimal))
                return Convert.ToDecimal(Dato);
            else
                return Dato;
        }

        public static void CopiarDatosDeUnaClaseAOtro(Object origen, Object destino)
        {
            try
            {
                foreach (PropertyInfo item in origen.GetType().GetProperties())
                {
                    destino.GetType().GetProperty(item.Name).SetValue(destino,
                        EntityUtils.ConvertirATipoDato(item.PropertyType, item.GetValue(origen, null)), null);
                }
            }
            catch (Exception exc)
            {
       ...
       #region numeros a letras
        ...
        public EntityUtils(Boolean ConvertirDecimales, String MascaraSalidaDecimal, String SeparadorDecimalSalida, Boolean LetraCapital)
        {
        ...
        {
            return EntityUtils.ToCardinal(Numero, CultureInfo.CurrentCulture);
        }
